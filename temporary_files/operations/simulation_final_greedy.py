...
class Drone(Point):
    ...
    def find_battery_and_time_required(self, targets: list[Point]) -> float:
        ...
        for j in range(len(targets)):
            ...
            time_needed = total_distance / self.speed * 1.2 + sum([target.waiting_time for target in short_targets[1:]])
            ...
            if j == len(short_targets) - 1:
                energy_needed = total_distance * self.energy_per_meter * 10 * 1.2 + constants.TO_land_energy * (len(short_targets) - 1)
        return energy_needed, time_needed, arrival_times, departure_times
    
    def check_planned_orders(self, path: list[Point], planned_orders: list[Order], arrival_times: list[float]):
        for planned in planned_orders:
            time_to_order = arrival_times[path.index(planned)-1]
            if self.simulation.timestamp + time_to_order > planned.arrival_time + constants.deliver_time_window or self.simulation.timestamp + time_to_order < planned.arrival_time:
                return False
        return True
    ...
    def greedy_heuristic(self):
        ...
        for order in orders:
            if not any(isinstance(target, Order) for target in self.targets):
                path = [first_node, order.restaurant, order, order.nearest(depots)]
                planned_orders = False
            else:
                planned_restaurants = [target for target in self.targets[1:] if isinstance(target, Restaurant)]
                planned_orders = [target for target in self.targets[1:] if isinstance(target, Order)]
                planned_orders.append(order)
                nodes = planned_restaurants
                nodes.append(order.restaurant)
                path = [first_node]
                removed_restaurants = []
                while nodes:
                    nearest_node = path[-1].nearest(nodes)
                    if isinstance(nearest_node, Restaurant):
                        n_pizzas_picked_up = sum([planned_order.demand for planned_order in planned_orders if planned_order.restaurant == nearest_node])
                        if n_pizzas_picked_up + self.load > self.max_capacity:
                            removed_restaurants.append(nearest_node)
                            nodes.remove(nearest_node)
                            continue
                    nodes.extend(removed_restaurants)
                    removed_restaurants = []
                    path.append(nearest_node)
                    nodes.remove(nearest_node)
                    if isinstance(nearest_node, Restaurant):
                        for planned_order in planned_orders:
                            if nearest_node == planned_order.restaurant:
                                nodes.append(planned_order)
                path.append(order.nearest(depots))
    
            energy_needed, _, arrival_times, departure_times = self.find_battery_and_time_required(path)
            if planned_orders:
                if not self.check_planned_orders(path, planned_orders, arrival_times):
                    continue
            time_to_order = arrival_times[path.index(order)-1]
            if self.battery >= energy_needed + 20 and self.simulation.timestamp + time_to_order <= order.arrival_time + constants.max_waiting_time and self.simulation.timestamp + time_to_order >= order.arrival_time:
                self.targets = []
                ...
                if self.depot is not None:
                    ...
                    self.depot = None 
                print(f"Drone {self.drone_id} assigned targets: {[node.name for node in self.targets]}")
    ...
    def update_drone(self, dt):
        if self.available_time < self.simulation.timestamp and self.depot is not None or self.departure_times and self.departure_times[0] <= self.simulation.timestamp and self.targets and self.state == 'waiting':
...
def animate_simulation(sim, steps=100, interval=200, save_path=None):
    ...
    fig.suptitle('Simulation Map Animation', fontsize=14)
    ...
    def update(frame):
        ...
        return (scat_orders, scat_drones, scat_restaurants, scat_depots, *path_lines, time_text)
...
if __name__ == "__main__":
    ...
    for i in range(n_steps):
        my_sim.take_step()
    #animate_simulation(my_sim, n_steps, interval=30)
