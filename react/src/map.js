...
import datajson from "./delft_boundary.json";
...
var MapPicker = function MapPicker(_ref) {
  ...
  function loadMap() {
    ...
    var validLocation = isValidLocation(defaultLocation)
      ? defaultLocation
      : {
          lat: 0,
          lng: 0,
    ...
    console.log("polygone created");
    flightPath.setMap(map.current);
    map.current.data.addGeoJson(datajson);
    map.current.data.setStyle({
      fillColor: "blue",
      strokeColor: "blue",
      fillOpacity: 0, // Polygon interior is transparent
      strokeOpacity: 0.8,
      strokeWeight: 5,
    });
