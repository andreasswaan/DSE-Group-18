...
mtow, oew, mtow_history, S_history, n_iterations, component_dict = drone.iterative_weight_estimate(
    transition=True, plot=False, max_iterations=100, tolerance=0.001
)
...
plt.tight_layout()
plt.savefig(
    os.path.join(plot_path, "simplest_mtow_convergence.pdf"),
    dpi=300,
    format="pdf",
    bbox_inches="tight",
)
...
np.savez(
    os.path.join(data_path, "simplest_convergence_results_insulated.npz"), **results_dict
)
